...
def sample_tenmat_4way():
    ...
    params = {'data':data, 'rdims':rdims, 'cdims':cdims, 'tshape':tshape, 'shape':shape}
...
@pytest.mark.indevelopment
def test_tenmat_initialization_from_data(sample_ndarray_1way, sample_ndarray_2way, sample_ndarray_4way, sample_tenmat_4way):
    ...
    tenmatNdarray1c = ttb.tenmat.from_data(np.reshape(ndarrayInstance1,(ndarrayInstance1.shape[0],1), order='F'), rdims, cdims, tshape)
    ...
    exc = 'Incorrect dimensions specified: products of data.shape and tuple do not match'
    with pytest.raises(AssertionError) as excinfo:
        a = ttb.tenmat.from_data(ndarrayInstance2, rdims, cdims, tuple(np.array(tshape) + 1))
    assert exc in str(excinfo)
...
def test_tenmat_initialization_from_tensor_type(sample_tenmat_4way, sample_tensor_4way):
    ...
    tshapeFC = (2, 2, 2, 2)
    shapeFC = (2, 8)
    dataFC = np.array([[ 1, 5,  9, 13, 2, 6, 10, 14], 
                       [ 3, 7, 11, 15, 4, 8, 12, 16]])
    tenmatTensorFC = ttb.tenmat.from_tensor_type(tensorInstance, rdims=rdimsFC, cdims_cyclic='fc')
    ...
    tshapeBC = (2, 2, 2, 2)
    shapeBC = (2, 8)
    dataBC = np.array([[ 1, 2,  9, 10, 5, 6, 13, 14], 
		               [ 3, 4, 11, 12, 7, 8, 15, 16]])
    tenmatTensorBC = ttb.tenmat.from_tensor_type(tensorInstance, rdims=rdimsBC, cdims_cyclic='bc')
    ...
    with pytest.raises(AssertionError) as excinfo:
        a = ttb.tenmat.from_tensor_type(tensorInstance, rdims=np.array([0,1,4]), cdims=cdims)
    assert exc in str(excinfo)

    # cdims must be valid dimensions
    exc = 'Values in cdims must be in [0, source.ndims].'
    with pytest.raises(AssertionError) as excinfo:
        a = ttb.tenmat.from_tensor_type(tensorInstance, rdims=rdims, cdims=np.array([2,3,4]))
    assert exc in str(excinfo)
...
def test_tenmat__mul__(sample_ndarray_1way, sample_ndarray_4way, sample_tenmat_4way):
    ...
    tenmat1 = ttb.tenmat.from_data(ndarrayInstance4, rdims=np.array([0]), cdims=np.array([1,2,3]))
    tenmat2 = ttb.tenmat.from_data(ndarrayInstance4, rdims=np.array([0,1,2]), cdims=np.array([3]))
    ...
    tenmat1 = ttb.tenmat.from_data(ndarrayInstance4, rdims=np.array([0,1]), cdims=np.array([2,3]))
    tenmat2 = ttb.tenmat.from_data(ndarrayInstance4, rdims=np.array([0,1,2]), cdims=np.array([3]))
    with pytest.raises(AssertionError) as excinfo:
        a = tenmat1 * tenmat2
    assert exc in str(excinfo)
...
def test_tenmat__add__(sample_ndarray_2way, sample_tenmat_4way):
    ...
    # Tenmat + Tenmat
    assert ((tenmatInstance + tenmatInstance).data == (params['data'] + params['data'])).all()
...
def test_tenmat__rsub__(sample_ndarray_2way, sample_tenmat_4way):
    ...
    # Tenmat + Tenmat
    assert ((tenmatInstance.__rsub__(tenmatInstance)).data == (params['data'] - params['data'])).all()
...
@pytest.mark.indevelopment
def test_tenmat__str__(sample_ndarray_1way, sample_ndarray_2way, sample_ndarray_4way, sample_tenmat_4way):
    (params, tenmatInstance) = sample_tenmat_4way
    tshape = params['tshape']
    rdims = params['rdims']
    cdims = params['cdims']
