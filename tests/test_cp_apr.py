...
def test_loglikelihood():
    ...
    fm0 = np.array([[0., 0.], [3., 4.]])
    fm1 = np.array([[0., 6.], [7., 8.]])
    ...
    assert np.isclose(explicitAnswer, ttb.tt_loglikelihood(sptensorInstance, ktensorInstance))
    assert np.isclose(explicitAnswer, ttb.tt_loglikelihood(tensorInstance, ktensorInstance))
...
def test_calculatePi():
    ...
    assert np.all(np.isclose(ttb.calculatePi(tensorInstance, ktensorInstance, 2, 0, tensorInstance.ndims), answer))
    assert np.all(np.isclose(ttb.calculatePi(sptensorInstance, ktensorInstance, 2, 0, sptensorInstance.ndims), answer))
...
def test_calculatePhi():
    ...
    assert np.isclose(ttb.calculatePhi(sptensorInstance, ktensorInstance, 2, 0, Pi, 1e-12), answer).all()
    assert np.isclose(ttb.calculatePhi(tensorInstance, ktensorInstance, 2, 0, Pi, 1e-12), answer).all()
...
def test_cpapr_pqnr(capsys):
    ...
    weights = np.array([1., 2.])
    fm0 = np.array([[1., 1.], [3., 4.]])
    fm1 = np.array([[1., 6.], [7., 8.]])
    factor_matrices = [fm0, fm1]
    ktensorInstance = ttb.ktensor.from_data(weights, factor_matrices)
    tensorInstance = ktensorInstance.full()
    np.random.seed(123)
...
def test_calculatepi_prowsubprob():
    ...
    assert np.all(np.isclose(ttb.tt_calcpi_prowsubprob(tensorInstance, ktensorInstance, 2, 0, tensorInstance.ndims), answer))
    assert np.all(np.isclose(ttb.tt_calcpi_prowsubprob(sptensorInstance, ktensorInstance, 2, 0, sptensorInstance.ndims, True, np.arange(sptensorInstance.subs.shape[0])), answer))
...
def test_calc_partials():
    ...
    phi, ups = ttb.calc_partials(False, Pi, 1e-12, tensorInstance[0, :].data, ktensorInstance[0][0, :])
    assert np.isclose(phi, np.array([0,0])).all()
    assert np.isclose(ups, np.array([0, 0])).all()

    phi, ups = ttb.calc_partials(False, Pi, 1e-12, tensorInstance[1, :].data, ktensorInstance[0][0, :])
    assert np.isclose(phi, 1e14*np.array([5.95, 9.68])).all()
    assert np.isclose(ups, 1e13*np.array([4.8, 8.5])).all()

    phi, ups = ttb.calc_partials(True, Pi, 1e-12, sptensorInstance.vals, ktensorInstance[0][0, :])
    assert np.isclose(phi, 1e14 * np.array([5.95, 9.68])).all()
    assert np.isclose(ups, 1e13 * np.array([4.8, 8.5])).all()
...
def test_getHessian():
    ...
    rank = 2
    sptensorInstance = ttb.sptensor.from_tensor_type(tensorInstance)
    Pi = ttb.calculatePi(sptensorInstance, ktensorInstance, rank, 0, tensorInstance.ndims)
    phi, ups = ttb.calc_partials(False, Pi, 1e-12, tensorInstance[1, :].data, ktensorInstance[0][0, :])
...
def test_getSearchDirPdnr():
    ...
    projGradStep = (model_row - ups.transpose()) * (model_row - (ups.transpose() > 0).astype(float))
...
def test_tt_loglikelihood_row():
    ...
    sptensorInstance = ttb.sptensor.from_tensor_type(tensorInstance)
    Pi = ttb.calculatePi(sptensorInstance, ktensorInstance, 2, 0, tensorInstance.ndims)
    loglikelihood = ttb.tt_loglikelihood_row(False, tensorInstance[1, :].data, tensorInstance[1, :].data, Pi)
    #print(loglikelihood)
...
def test_tt_linesearch_prowsubprob():
    ...
    Pi = ttb.calculatePi(sptensorInstance, ktensorInstance, 2, 0, tensorInstance.ndims)
    phi, ups = ttb.calc_partials(False, Pi, 1e-12, tensorInstance[1, :].data, ktensorInstance[0][0, :])
    search, pred = ttb.getSearchDirPdnr(Pi, ups, 2, phi, tensorInstance[1, :].data, 0.1, 1e-6)
    with pytest.warns(Warning) as record:
        ttb.tt_linesearch_prowsubprob(search.transpose()[0], phi.transpose(), tensorInstance[1, :].data, 1, 1 / 2, 10,
                              1.0e-4, False, tensorInstance[1, :].data, Pi, phi, True)
    assert 'CP_APR: Line search failed, using multiplicative update step' in str(record[0].message)
