...
def sample_ktensor_3way():
    ...
    data = {'weights': weights, 'factor_matrices': factor_matrices, "vector": vector,
            "vector_with_weights": vector_with_weights, "shape": shape}
    ktensorInstance = ttb.ktensor.from_data(weights, factor_matrices)
    return data, ktensorInstance
...
def sample_ktensor_symmetric():
    ...
    fm0 = np.array([[2.340431417384394, 4.951967353890655],
                    [4.596069112758807, 8.012451489774961]])
    fm1 = np.array([[2.340431417384394, 4.951967353890655],
                    [4.596069112758807, 8.012451489774961]])
    factor_matrices = [fm0, fm1]
    data = {'weights': weights, 'factor_matrices': factor_matrices}
    ktensorInstance = ttb.ktensor.from_data(weights, factor_matrices)
...
def test_ktensor_from_factor_matrices(sample_ktensor_2way):
    ...
    K1 = ttb.ktensor.from_factor_matrices(data["factor_matrices"][0], data["factor_matrices"][1])
...
def test_ktensor_from_data(sample_ktensor_2way, capsys):
    ...
    # Create ktensor with weights and multiple factor matrices as arguments
    K1 = ttb.ktensor.from_data(data["weights"], data["factor_matrices"][0], data["factor_matrices"][1])
...
def test_ktensor_from_function():
    ...
    assert (K0.weights == np.array([1., 1.])).all()
    ...
    assert (K1.weights == np.array([1., 1.])).all()
    fm0 = np.array(
...
def test_ktensor_arrange(sample_ktensor_2way):
    ...
    assert "Weighting and permuting the ktensor at the same time is not allowed." in str(excinfo)

    # error, length of permutation must equal number of components in ktensor
    with pytest.raises(AssertionError) as excinfo:
        K1.arrange(permutation=[0,1,2])
    assert "Number of elements in permutation does not match number of components in ktensor." in str(excinfo)
...
def test_ktensor_double(sample_ktensor_2way, sample_ktensor_3way):
    ...
    assert (K2.double() == np.array([[29., 39.], [63., 85.]])).all()
    (data3, K3) = sample_ktensor_3way
    A = np.array([ 830.,  888.,  946., 1004.,  942., 1008., 1074., 1140., 1054., 1128., 1202., 1276.,
                  1180., 1264., 1348., 1432., 1344., 1440., 1536., 1632., 1508., 1616., 1724., 1832.]).reshape((2, 3, 4))
...
def test_ktensor_extract(sample_ktensor_3way):
    ...
    with pytest.raises(AssertionError) as excinfo:
        K.extract(1.0)
    assert "Input parameter must be an int, tuple, list or numpy.ndarray" in str(excinfo)

    # too many components
    with pytest.raises(AssertionError) as excinfo:
        K.extract([0,1,2,3])
    assert "Number of components requested is not valid: 4 (should be in [1,...,2])." in str(excinfo)

    # component index out of range
    with pytest.raises(AssertionError) as excinfo:
        K.extract((5))
    assert "Invalid component indices to be extracted: [5] not in range(2)" in str(excinfo)
...
def test_ktensor_fixsigns(sample_ktensor_2way):
    ...
    factor_matrix10 = np.array([[ 0.3162277660168379, -0.4472135954999579],
                                [ 0.9486832980505138, -0.8944271909999159]])
    factor_matrix11 = np.array([[0.5812381937190965, -0.6               ],
                                [0.813733471206735,  -0.8               ]])
...
def test_ktensor_mttkrp(sample_ktensor_3way):
    ...
    output0 = np.array([[12492., 12492., 12492., 12492.],
                       [17856., 17856., 17856., 17856.]])
    assert (K.mttkrp(K1.factor_matrices, 0) == output0).all()
    output1 = np.array([[ 8892.,  8892.,  8892.,  8892],
                        [10116., 10116., 10116., 10116],
                        [11340., 11340., 11340., 11340]])
    assert (K.mttkrp(K1.factor_matrices, 1) == output1).all()
    output2 = np.array([[6858., 6858., 6858., 6858],
                        [7344., 7344., 7344., 7344],
                        [7830., 7830., 7830., 7830],
                        [8316., 8316., 8316., 8316]])
    ...
    fm_wrong_size = [K1.factor_matrices[0], K1.factor_matrices[0], K1.factor_matrices[0], np.ones((5, 4))]
    with pytest.raises(AssertionError) as excinfo:
        K.mttkrp(fm_wrong_size, 0)
    assert 'List of factor matrices is the wrong length' in str(excinfo)\

    # Wrong input type
    fm_wrong_type = (K1.factor_matrices[0], K1.factor_matrices[0], K1.factor_matrices[0])
    with pytest.raises(AssertionError) as excinfo:
...
def test_ktensor_norm():
    ...
    assert pytest.approx(K2.norm(), 1e-8) == 6.337788257744180e+03
...
def test_ktensor_normalize(sample_ktensor_2way, sample_ktensor_3way):
    ...
    factor_matrix0 = np.array([[0.4767312946227962, 0.5111012519999519],
    ...
    factor_matrix10 = np.array([[0.4472135954999579, 0.6               ],
                                [0.8944271909999159, 0.8               ]])
    factor_matrix11 = np.array([[0.4767312946227962, 0.5111012519999519],
                                [0.5720775535473555, 0.5749889084999459],
                                [0.6674238124719146, 0.6388765649999399]])
    factor_matrix12 = np.array([[0.4382504900892777, 0.4535055413676754],
    ...
    #print("\nK2\n",K2)
    weights2 = np.array([5400., 24948.])
    factor_matrix20 = np.array([[0.3333333333333333, 0.4285714285714285],
                                [0.6666666666666666, 0.5714285714285714]])
    factor_matrix21 = np.array([[0.2777777777777778, 0.2962962962962963],
                                [0.3333333333333333, 0.3333333333333333],
                                [0.3888888888888888, 0.3703703703703703]])
    factor_matrix22 = np.array([[0.22,               0.2272727272727273],
    ...
    factor_matrix30 = np.array([[0.4472135954999579, 0.6000000000000001],
                                [0.8944271909999159, 0.8               ]])
    factor_matrix31 = np.array([[ 561.2486080160912, 2646.0536653665963],
    ...
    with pytest.raises(AssertionError) as excinfo:
        K0.normalize(mode=4)
    assert "Parameter single_factor is invalid; index must be an int in range of number of dimensions" in str(excinfo)

    # normalize and sort
    K4.normalize(sort=True)
    weights = np.array([5177.161384388167, 1177.285012220915])
    fm0 = np.array([[0.6000000000000001, 0.4472135954999579],
                   [0.8, 0.8944271909999159]])
    fm1 = np.array([[0.5111012519999519, 0.4767312946227962],
                    [0.5749889084999459, 0.5720775535473555],
                    [0.6388765649999399, 0.6674238124719146]])
    fm2 = np.array([[0.4535055413676753, 0.4382504900892776],
    ...
    weights = np.array([1., 1.])
    fm0 = np.array([[1.6493314105258194, 4.229485053762256 ],
                   [4.947994231577458,  8.458970107524513 ]])
    fm1 = np.array([[3.031531424242968, 5.674449654019056],
...
def test_ktensor_nvecs(sample_ktensor_3way):
    ...
    with pytest.warns(Warning) as record:
        assert np.allclose(K.nvecs(0, 1), np.array([[0.5731077440321353],
                                                    [0.8194800264377384]]))
    assert 'Greater than or equal to ktensor.shape[n] - 1 eigenvectors requires cast to dense to solve' in str(
        record[0].message)
    with pytest.warns(Warning) as record:
    ...
    assert np.allclose(K.nvecs(1, 1), np.array([[0.5048631426517823],
                                                [0.5745404391632514],
                                                [0.6442177356747206]]))
    with pytest.warns(Warning) as record:
        assert np.allclose(K.nvecs(1, 2), np.array([[ 0.5048631426517821,  0.7605567306550753],
                                                    [ 0.5745404391632517,  0.0568912743440822],
                                                    [ 0.6442177356747206, -0.6467741818894517]]))
    assert 'Greater than or equal to ktensor.shape[n] - 1 eigenvectors requires cast to dense to solve' in str(
        record[0].message)

    assert np.allclose(K.nvecs(2, 1), np.array([[0.4507198734531968],
                                                [0.4827189140450413],
                                                [0.5147179546368857],
                                                [0.5467169952287301]]))
    assert np.allclose(K.nvecs(2, 2), np.array([[ 0.4507198734531969,  0.7048770074600103],
    ...
    with pytest.warns(Warning) as record:
        K.nvecs(1, 3)
    assert 'Greater than or equal to ktensor.shape[n] - 1 eigenvectors requires cast to dense to solve' in str(record[0].message)
...
def test_ktensor_symmetrize(sample_ktensor_2way):
    ...
    K1 = K.symmetrize()
    weights = np.array([1., 1.])
    fm = np.array([[2.340431417384394,4.951967353890656],
    ...
    K3 = K2.symmetrize()
    out_fm = np.array([[-1., -1.2599210498948732, 1.442249570307408],
                       [-1., -1.2599210498948732, 1.442249570307408],
                       [-1., -1.2599210498948732, 1.442249570307408],
                       [-1., -1.2599210498948732, 1.442249570307408]])
...
def test_ktensor_tolist(sample_ktensor_3way):
    ...
    fm0 = K.tolist()
    m0 = np.array([[1.2599210498948732, 3.7797631496846193],
                   [2.5198420997897464, 5.039684199579493]])
    m1 = np.array([[ 6.299605249474366, 10.079368399158986],
                   [ 7.559526299369239, 11.339289449053858],
                   [ 8.819447349264113, 12.599210498948732]])
    m2 = np.array([[13.859131548843605, 18.898815748423097],
    ...
    fm1 = K.tolist(0)
    m0 = np.array([[ 526.4978632435273, 3106.2968306329008],
                   [1052.9957264870545, 4141.729107510534 ]])
    m1 = np.array([[0.4767312946227962, 0.5111012519999519],
                   [0.5720775535473555, 0.5749889084999459],
                   [0.6674238124719146, 0.6388765649999399]])
...
def test_ktensor_ttv(sample_ktensor_3way):
    ...
    weights = np.array([36., 54.])
    fm0 = np.array([[1., 3.], [2., 4.]])
    fm1 = np.array([[11., 15.], [12., 16.], [13., 17.], [14., 18.]])
    factor_matrices = [fm0, fm1]
...
def test_ktensor_update(sample_ktensor_3way):
    ...
    assert 'Failed to consume all of the input data' \
...
def test_ktensor__add__(sample_ktensor_2way, sample_ktensor_3way):
    ...
    for k in range(K2.ndims):
        assert (np.concatenate((data0["factor_matrices"][k], data0["factor_matrices"][k]), axis=1) == K2.factor_matrices[k]).all()
...
def test_ktensor__getitem__(sample_ktensor_2way):
    ...
    with pytest.raises(AssertionError) as excinfo:
        K[5]
    assert 'ktensor.__getitem__() can only extract single elements (tuple of indices) or factor matrices (single index)' in str(excinfo)
...
def test_ktensor__setitem__(sample_ktensor_2way):
    ...
    with pytest.raises(AssertionError) as excinfo:
        K[0, 0] = 1
    assert "Subscripted assignment cannot be used to update individual elements of a ktensor." in str(excinfo)
...
def test_ktensor__mul__(sample_ktensor_2way, sample_ktensor_3way):
    ...
    with pytest.raises(AssertionError) as excinfo:
        K3 = K0 * K0
    assert "Multiplication by ktensors only allowed for scalars, tensors, or sptensors" in str(excinfo)
    ...
    T = ttb.tensor().from_data(Tdata, Tshape)
    K0T = K0 * T
    assert (K0T.double() == np.array([[29., 78.], [189., 340.]])).all()
    ...
    S = ttb.sptensor().from_data(Ssubs, Svals, Sshape)
    K0S = S * K0
    assert (K0S.double() == np.array([[14.5, 39.], [0., 127.5]])).all()
