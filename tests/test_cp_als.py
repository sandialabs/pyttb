...
from pyttb.decompositions.cp import cp_als
...
def sample_ttensor():
    ...
    core = tensor(np.ones((2, 3)))
    factors = [
        np.ones((5, 2)),
        np.ones((2, 3)),
    ]
    ttensorInstance = ttensor(core, factors)
...
def random_ttensor():
    ...
    core = tensor(np.random.random((2, 3, 4)))
    factors = [
...
def test_cp_als_tensor_nvecs_init(capsys, sample_tensor):
    (data, T) = sample_tensor
    (M, Minit, output) = cp_als(T, 1, init="nvecs")
    capsys.readouterr()
...
def test_cp_als_tensor_ktensor_init(capsys, sample_tensor):
    (data, T) = sample_tensor
    KInit = ktensor.from_function(np.random.random_sample, T.shape, 2)
    (M, Minit, output) = cp_als(T, 2, init=KInit)
    capsys.readouterr()
    assert pytest.approx(output["fit"]) == 1
...
def test_cp_als_incorrect_init(capsys, sample_tensor):
    ...
    with pytest.raises(AssertionError) as excinfo:
        (M, Minit, output) = cp_als(T, 2, init="init")
    ...
    KInit = ktensor.from_function(np.random.random_sample, Tshape_incorrect, 2)
    with pytest.raises(AssertionError) as excinfo:
        (M, Minit, output) = cp_als(T, 2, init=KInit)
...
def test_cp_als_ttensor_default_init_consistency(capsys, random_ttensor):
    T = random_ttensor
    KInit = ktensor.from_function(np.random.random_sample, T.shape, 2)
    _, _, output = cp_als(T, 2, init=KInit)
    capsys.readouterr()
    _, _, dense_output = cp_als(T.full(), 2, init=KInit)
    capsys.readouterr()
...
def test_cp_als_tensor_dimorder(capsys, sample_tensor):
    ...
    (M, Minit, output) = cp_als(T, 2, dimorder=dimorder)
    capsys.readouterr()
    assert pytest.approx(output["fit"]) == 1
    ...
    # dimorder not a list
    with pytest.raises(AssertionError) as excinfo:
        (M, Minit, output) = cp_als(T, 2, dimorder=2)
...
def test_cp_als_tensor_zeros(capsys, sample_tensor):
    # 2-way tensor
    T2 = tensor.from_function(np.zeros, (2, 2))
    (M2, Minit2, output2) = cp_als(T2, 2)
    capsys.readouterr()
    assert pytest.approx(output2["fit"], 1) == 0
    assert output2["normresidual"] == 0

    # 3-way tensor
    T3 = tensor.from_function(np.zeros, (3, 4, 5))
    (M3, Minit3, output3) = cp_als(T3, 2)
    capsys.readouterr()
...
def test_cp_als_sptensor_zeros(capsys):
    # 2-way tensor
    shape2 = (2, 2)
    T2 = sptensor.from_function(np.zeros, shape2, np.ceil(np.prod(shape2) / 2.0))
    print(T2)
    (M2, Minit2, output2) = cp_als(T2, 2)
    capsys.readouterr()
    assert pytest.approx(output2["fit"], 1) == 0
    ...
    # 3-way tensor
    shape3 = (2, 2)
    T3 = sptensor.from_function(np.zeros, shape3, np.ceil(np.prod(shape3) / 2.0))
    (M3, Minit3, output3) = cp_als(T3, 2)
    capsys.readouterr()
    assert pytest.approx(output3["fit"], 1) == 0
    assert output3["normresidual"] == 0
...
def test_cp_als_tensor_pass_params(capsys, sample_tensor):
    ...
    _, _, output = cp_als(T, 2, init=KInit, maxiters=2)
    capsys.readouterr()

    # passing the same parameters back to the method will yield the exact same results
    _, _, output1 = cp_als(T, 2, init=KInit, **output["params"])
    capsys.readouterr()

    # changing the order should also work
    _, _, output2 = cp_als(T, 2, **output["params"], init=KInit)
    capsys.readouterr()
...
def test_cp_als_tensor_printitn(capsys, sample_tensor):
    ...
    # default printitn
    cp_als(T, 2, printitn=1, maxiters=2)
    capsys.readouterr()

    # zero printitn
    cp_als(T, 2, printitn=0, maxiters=2)
    capsys.readouterr()
    ...
    # float printitn
    cp_als(T, 2, printitn=1.5, maxiters=2)
    capsys.readouterr()
